...
def send_alerts_if_needed(metrics, *args, **kwargs):
    ...
    if 'data_quality' in metrics:
        ...
        if avg_score < 70:
            alert_msg = f"CALIDAD DE DATOS BAJA - Score promedio: {avg_score:.1f}"
            print(alert_msg)
            alerts.append({
        ...
        if total_trips > 0:
            ...
            if valid_ratio < 0.8:
                alert_msg = f"ALTA TASA DE DATOS INVÁLIDOS - {valid_trips}/{total_trips} válidos ({valid_ratio:.1%})"
                print(alert_msg)
                alerts.append({
    ...
    # 3. Verificar cola SQS saturada
    if 'sqs_metrics' in metrics:
        message_count = metrics['sqs_metrics'].get('message_count', 0)
    ...
    # 4. Guardar métricas en S3 para dashboard
    try:
        s3 = boto3.client(
            's3',
